...
const services = [
  ...
  { number: '03', title: 'Professional massage & wellness', text: 'A calm, professional, sexual massage and wellness experience designed around relaxation and restorative touch. We discuss comfort, pressure, accessibility, and the setting before every appointment.' },
]
    ...
    <main>
      ...
      <section className="hero" id="top">
        <div className="hero-copy">
          ...
          <p className="hero-intro">Companios provides male companions in Bhopal for casual dating, dinners, parties, social outings, and professional happy massages. Every experience is arranged with discretion, clear communication, and respect.</p>
      ...
      <section className="contact section-pad" id="contact" aria-labelledby="contact-title"><div className="contact-intro"><p className="eyebrow">A private first step</p><h2 id="contact-title">Tell us what<br /><em>you have in mind.</em></h2><p>Looking for a male companion for casual dating, a party or event, or a professional massage in Bhopal? Escort services are available. Contact us easily by email or use the private enquiry form.</p><a className="contact-email" href="mailto:hello.bookaman@gmail.com">hello.bookaman@gmail.com <ArrowUpRight aria-hidden="true" /></a><a className="contact-email" href="tel:+919993553513">Direct booking: 9993553513 <ArrowUpRight aria-hidden="true" /></a><p className="contact-small">We aim to respond within one business day.<br />Your details are treated confidentially.</p></div><form className="contact-form" onSubmit={handleSubmit} aria-label="Private enquiry form">{submitted ? <div className="success-message"><Check aria-hidden="true" /><h3>Thank you for reaching out.</h3><p>Your enquiry has been sent. We will be in touch with next steps.</p></div> : <><label>Name<input required name="name" autoComplete="name" maxLength={120} placeholder="Your name" /></label><label>Email<input required type="email" name="email" autoComplete="email" maxLength={254} placeholder="you@example.com" /></label><label className="honeypot" aria-hidden="true">Website<input name="website" tabIndex={-1} autoComplete="off" /></label><label>What can we help with?<textarea required name="message" rows={4} maxLength={4000} placeholder="A date, an occasion, or simply a little more about what you are looking for..." /></label>{error && <p role="alert" className="form-error">{error}</p>}<button className="button-link" type="submit" disabled={sending}>{sending ? 'Sending enquiry...' : 'Send private enquiry'} {!sending && <ArrowUpRight aria-hidden="true" />}</button></>}</form></section>

      <footer className="site-footer"><a className="wordmark" href="#top">companios<span>.</span></a><p>Male dating, party companionship, and professional massage in Bhopal.<br /><a href="mailto:hello.bookaman@gmail.com">hello.bookaman@gmail.com</a><br /><a href="tel:+919993553513">Direct booking: 9993553513</a></p><div><a href="#services">Services</a><a href="#faq">FAQ</a><a href="#contact">Contact</a></div><small>© {new Date().getFullYear()} Companios. All rights reserved.</small></footer>
    </main>
  )
}
